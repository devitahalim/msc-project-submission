...
currentDirectory = '/Users/devitahalim/Documents/GitHub/VRPT_Project/varying_frequency/'
...
instanceList = ['AuPost_Central_2019'] #ALREADY 3 SLOTS
instanceList = ['AuPost_Northern_2019','AuPost_Western_2019', 'AuPost_Southern_2019'] 

alpha_list = [1, 0.99, 0.98, 0.95]
frequency_list = [3, 5, 10, 20]
frequency_list = [2/2, 3/2, 5/2, 10/2]
...
for file_name in instanceList:
    for f in frequency_list:
        mod = 'Stochastic'
        # Read instance file
        scenario_dict, n_scenario, numVehicles, numNodes, vehicles, nPar, nodeDistances, nodeStorage,times, loadings, productOD, productSize, VCd,expected_times, expected_loadings= read_in_AP(dataDirectory, file_name, mod, frequency= f)
        print(productSize)
        ...
        if run:
            for alpha in alpha_list:
                for tr_case in [0, 2]:
                    print('---------------------------------------')
                    print(f'Running {case}, tr_case {tr_case}, with alpha {alpha}, frequency = {f*2}:')
                    print('---------------------------------------')
                    ...
                    resultFileName= str(tr_case) +'Freq'+str(f)+'_alpha'+str(alpha)+ file_name  + ".npz"
        ...
        if analyse:
            for alpha in alpha_list:
                for tr_case in [0,2]:
                    ...
                    if load:
                        load_file = currentDirectory +'results/' + str(tr_case) +'Freq'+str(f)+'_alpha'+str(alpha)+ file_name  + ".npz"
                    ...
                    if plot:
                        ...
                        for s in range(n_scenario): #  selected scenarios
                            ...
                            if int(cc_2_prime_array[s]) == 0:
                                    ax.set_title(f'Scenario {s} (FAILURE): Traffic {traffic}, Demand {dem}, Loading {load}, \u03B1={alpha}, Freq {round(f*2)}')
                            else:
                                ax.set_title(f'Scenario {s}: Traffic {traffic}, Demand {dem}, Loading {load}, \u03B1={alpha}, Freq {round(f*2)}')
                            ...
                            plt.savefig(os.path.join(save_dir, f'{tr_case}_Freq_{f*3}_{file_name}_alpha_{alpha}_scenario_{s}.png'))
                            plt.close(fig)
