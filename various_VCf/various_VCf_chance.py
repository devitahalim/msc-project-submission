...
currentDirectory = '/Users/devitahalim/Documents/GitHub/VRPT_Project/various_VCf/'
...
# VCf_list = [15, 20, 25, 30, 40, 50, 60, 70, 80]
VCf_list = [60, 80]
            ...
            for alpha in alpha_list:
                for tr_case in [0, 2]:
                    ...
                    d_prime_array, q_array, q_scenario_array, demand_array, cc_array, cc_prime_array, cc_2_prime_array, e_array\
                        ] = gurobiOpt(transhipment_coef, alpha, VCf)

                    time_end = time.time()
                    print("Takes " + str(time_end - time_start) + " seconds")

                    resultFileName= str(tr_case) +'VCF_'+str(VCf)+'_alpha'+str(alpha)+ file_name  + ".npz"
    ...
    if analyse:
        for alpha in alpha_list:
            for tr_case in [0,2]:
                ...
                if load:
                    load_file = currentDirectory +'results/' + str(tr_case) +'VCF_'+str(VCf)+'_alpha'+str(alpha)+ file_name  + ".npz"
                ...
                if plot:
                    ...
                    for s in range(n_scenario): #  selected scenarios
                        ...
                        plt.savefig(os.path.join(save_dir, f'{tr_case}VCF_{VCf}_alpha{alpha}_{file_name} _scenario_{s}.png'))
                        plt.close(fig)
